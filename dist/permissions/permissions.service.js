...
let PermissionsService = class PermissionsService {
    ...
    async findAll() {
        return this.permissionModel.find({ isDeleted: false }).lean();
    }
    async findOne(id) {
    ...
    async update(id, updatePermissionDto) {
        const permission = await this.permissionModel
            .findOneAndUpdate({ _id: id, isDeleted: false }, updatePermissionDto, { new: true })
            .lean();
        if (!permission)
            throw new common_1.NotFoundException(`Permission ${id} not found`);
        return permission;
    ...
    async remove(id) {
        ...
        if (!permission)
            throw new common_1.NotFoundException(`Permission ${id} not found`);
        return { message: `Permission ${id} has been deleted` };
    }
};
